...
#         Add a section heading that matches our name.
  ...
  def add_environment_resources(catalog, sections)
    ...
    if configured_environment == "production" && !path.empty? && File.exists?(path)
      configured_environment_path = File.join(path, configured_environment)
      catalog.add_resource(Puppet::Resource.new(:file, configured_environment_path, :parameters => { :ensure => 'directory' }))
    end
  end
